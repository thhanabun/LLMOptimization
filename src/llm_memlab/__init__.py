"""Memory-first LLM analysis toolkit."""

__version__ = "0.1.0"

from .attention_debugger import AttentionStats, analyze_qk_attention, attention_stats_to_text, collect_attention_stats
from .backend_registry import BackendInfo, BackendPlugin, BackendRegistry, default_backend_registry, load_backend_entrypoints
from .benchmark import BenchmarkConfig, BenchmarkResult, benchmark_callable, benchmark_decode, benchmark_forward, compare_benchmarks
from .benchmark_store import (
    BENCHMARK_SCHEMA_VERSION,
    BenchmarkComparison,
    BenchmarkGateConfig,
    BenchmarkGateResult,
    BenchmarkHistory,
    BenchmarkMetadata,
    BenchmarkRecord,
    assert_no_regressions,
    benchmark_history,
    benchmark_history_gate,
    benchmark_gate,
    collect_run_metadata,
    compare_benchmark_records,
    compare_record_sets,
    read_benchmark_csv,
    read_benchmark_files,
    read_benchmark_json,
    record_from_benchmark,
    records_from_suite,
    write_benchmark_csv,
    write_benchmark_json,
)
from .benchmark_dashboard import BenchmarkDashboard, benchmark_dashboard_from_files, write_benchmark_dashboard_html
from .benchmark_suite import InferenceSuiteResult, benchmark_inference_suite, compare_inference_suites
from .bytes import dtype_size_bytes, format_bytes, parse_bytes
from .compare_report import CompareReport, compare_report_to_html, scoreboard_to_html, write_compare_html, write_scoreboard_html
from .backends.cutile import CuTileDispatchResult, CuTileRuntimeInfo, certify_cutile_decode_attention, detect_cutile_runtime
from .backends.vllm import VLLMGenerateResult, VLLMRuntimeInfo, detect_vllm_runtime, run_vllm_generate, vllm_backend_plugin
from .decode_benchmarks import DecodeBackendMatrix, FusedDecodeBenchmark, benchmark_decode_backend_matrix, benchmark_fused_decode_attention
from .deep_debugger import DeepDebugReport, build_deep_debug_report, write_deep_debug_html
from .debugger_v2 import DEBUGGER_V2_SCHEMA_VERSION, DebuggerV2Report, debugger_v2_to_html, write_debugger_v2_html
from .env_certification import ENV_CERTIFICATION_SCHEMA_VERSION, EnvironmentCertificationReport, certify_environment
from .certification_matrix import (
    CERTIFICATION_MATRIX_SCHEMA_VERSION,
    CertificationMatrixGateResult,
    CertificationMatrixReport,
    ModelCertificationOutcome,
    ModelCertificationTarget,
    certify_model_matrix,
    default_model_certification_targets,
)
from .drift_debugger import DriftReport, LayerDriftRecord, compare_layer_drift
from .estimates import MemoryEstimate, TransformerConfig, estimate_transformer_memory, preset_config
from .hardware import HARDWARE_PROFILE_SCHEMA_VERSION, HardwareProfile, detect_hardware_profile
from .hf_adapter import (
    BaseFamilyMemoryAdapter,
    DeepSeekMemoryAdapter,
    FamilyAdapterCapabilities,
    FalconMemoryAdapter,
    GemmaMemoryAdapter,
    GPTNeoXMemoryAdapter,
    HFAdapterInfo,
    HFGenerateIntegrationPlan,
    LlamaMemoryAdapter,
    MemoryFirstGenerateResult,
    MemoryFirstHFAdapter,
    MemoryFirstHFConfig,
    MemoryFirstTransformersCache,
    MistralMemoryAdapter,
    MixtralMemoryAdapter,
    PhiMemoryAdapter,
    Qwen3MemoryAdapter,
    QwenMemoryAdapter,
    adapter_satisfies_contract,
    detect_hf_adapter_info,
    install_memory_first_generate,
    list_memory_adapters,
    make_transformers_cache_from_model,
    memory_first_generate,
    memory_first_generate_hf,
    register_memory_adapter,
    select_memory_adapter,
    supports_transformers_cache_api,
)
from .hf_adapter_contract import MemoryAdapterProtocol
from .hf_adapter_matrix import (
    HF_ADAPTER_MATRIX_SCHEMA_VERSION,
    HFAdapterMatrixEntry,
    hf_adapter_matrix_to_text,
    production_hf_adapter_matrix,
)
from .hf_cache_certification import (
    HF_CACHE_CERTIFICATION_SCHEMA_VERSION,
    HFCacheCertificationCase,
    HFCacheCertificationReport,
    HFCacheCertificationResult,
    assert_hf_cache_certified,
    certify_hf_cache,
    certify_hf_cache_model,
    hf_cache_certification_to_html,
)
from .hf_cache_policy import HFCachePolicy, HFCachePolicyDecision, select_hf_cache_policy
from .hf_cache_profiles import (
    CacheProfileRegistry,
    DEFAULT_QUANTIZED_CACHE_PROFILES,
    HF_CACHE_PROFILE_SCHEMA_VERSION,
    QuantizedCacheCertificationProfile,
    load_quantized_cache_profiles,
    promote_profile,
    select_quantized_cache_profile,
    write_quantized_cache_profiles,
)
from .hf_cache import HFCachePlan, plan_hf_cache
from .hf_runtime import HFGenerateRun, HFMemoryFirstBenchmark, assert_hf_benchmark_passed, benchmark_memory_first_hf_generate
from .html_report import (
    trace_interactive_to_html,
    trace_timeline_to_html,
    trace_to_html,
    write_interactive_html,
    write_timeline_html,
    write_trace_html,
)
from .inspector import ModelArchitectureInfo, inspect_model, load_hf_model
from .ir import GraphSpec, OperationSpec, TensorSpec
from .kernel_certification import (
    CERTIFICATION_SCHEMA_VERSION,
    KernelCertificationCase,
    KernelCertificationReport,
    KernelCertificationResult,
    certify_quantized_attention,
    default_kernel_certification_cases,
)
from .kernel_policy import KernelPolicy, KernelSelection, default_kernel_policy, select_kernel_policy
from .kernel_promotion import KERNEL_PROMOTION_SCHEMA_VERSION, KernelPromotionDecision, KernelPromotionRequirements, decide_kernel_promotion
from .local_model_harness import (
    DEFAULT_LOCAL_MODEL_FIXTURES,
    LOCAL_MODEL_FIXTURE_SCHEMA_VERSION,
    LocalModelFixture,
    LocalModelHarnessReport,
    LocalModelMatch,
    scan_local_model_fixtures,
)
from .kernels import (
    KernelConfig,
    QuantizedAttentionDispatch,
    apply_rope,
    chunked_cross_entropy,
    kernel,
    linear_cross_entropy,
    qkv_rope_attention,
    qkv_rope_attention_cached,
    quantized_kv_attention,
    select_quantized_attention_backend,
    rms_norm,
    rms_norm_manual_backward,
    scaled_dot_product_attention,
    swiglu,
    triton_apply_rope,
    triton_dequantize_int8_per_token,
    triton_dequantize_uint8_per_token,
    triton_fused_int8_kv_attention,
    triton_fused_uint8_kv_attention,
    triton_fused_int8_paged_kv_attention,
    triton_fused_uint8_paged_kv_attention,
    triton_quantize_int8_per_token,
    triton_quantize_uint8_per_token,
    triton_rms_norm,
    triton_swiglu_activation,
)
from .kv_cache import (
    DecodeConfig,
    DecodeResult,
    KVCacheConfig,
    PagedKVCache,
    QuantizedStaticKVCache,
    StaticKVCache,
    dequantize_int8_per_token,
    dequantize_uint8_per_token,
    greedy_decode,
    quantize_int8_per_token,
    quantize_uint8_per_token,
    resolve_kv_storage_dtype,
    sample_next_token,
)
from .kv_quality import (
    AttentionKVQualityResult,
    KVQualityResult,
    evaluate_attention_kv_quality,
    evaluate_int8_kv_quality,
    evaluate_kv_quantization_quality,
)
from .memory_policy import MemoryPolicy, choose_memory_policy
from .oom_runner import OOMRunResult, OOMStrategy, default_generation_strategies, is_oom_error, run_with_oom_fallback
from .optimization_report import OptimizationFinding, OptimizationReport, infer_findings
from .patchers import PackedQKVAttentionAdapter, PatchReport, optimize_hf_model, optimize_llama_qwen_attention
from .planner import BufferPlan, MemoryPlanner, TensorLifetime
from .quality_metrics import LogitQualityResult, TokenQualityResult, compare_logits, compare_token_sequences
from .quality_suite import (
    QUALITY_SCHEMA_VERSION,
    QualityRegressionResult,
    QualityThresholds,
    assert_quality_regression,
    run_quality_regression,
)
from .serving_benchmark import SERVING_BENCHMARK_SCHEMA_VERSION, ServingBenchmarkResult, ServingRun, benchmark_serving_paths
from .memory_profiler import (
    MEMORY_PROFILE_SCHEMA_VERSION,
    CUDAMemorySample,
    DecodeMemoryProfile,
    MemoryAttribution,
    MemoryProfileComparison,
    compare_memory_profiles,
    profile_decode_memory,
    write_memory_profile_html,
    write_memory_profile_json,
)
from .torch_debugger import TorchTrace, trace_forward
from .triton_kernels import triton_available

__all__ = [
    "AttentionKVQualityResult",
    "AttentionStats",
    "BackendInfo",
    "BackendPlugin",
    "BackendRegistry",
    "BenchmarkConfig",
    "BenchmarkDashboard",
    "CERTIFICATION_SCHEMA_VERSION",
    "CERTIFICATION_MATRIX_SCHEMA_VERSION",
    "write_memory_profile_json",
    "select_memory_adapter",
    "benchmark_gate",
    "benchmark_history",
    "benchmark_history_gate",
    "MEMORY_PROFILE_SCHEMA_VERSION",
    "QualityThresholds",
    "QUALITY_SCHEMA_VERSION",
    "QwenMemoryAdapter",
    "Qwen3MemoryAdapter",
    "SERVING_BENCHMARK_SCHEMA_VERSION",
    "MistralMemoryAdapter",
    "MixtralMemoryAdapter",
    "LlamaMemoryAdapter",
    "BaseFamilyMemoryAdapter",
    "BenchmarkGateResult",
    "BenchmarkHistory",
    "BenchmarkGateConfig",
    "BENCHMARK_SCHEMA_VERSION",
    "BenchmarkComparison",
    "BenchmarkMetadata",
    "BenchmarkRecord",
    "BenchmarkResult",
    "BufferPlan",
    "CacheProfileRegistry",
    "CertificationMatrixGateResult",
    "CertificationMatrixReport",
    "CompareReport",
    "QualityRegressionResult",
    "MemoryProfileComparison",
    "MemoryAttribution",
    "MemoryFirstTransformersCache",
    "MemoryAdapterProtocol",
    "KernelSelection",
    "HFAdapterInfo",
    "HFAdapterMatrixEntry",
    "HF_ADAPTER_MATRIX_SCHEMA_VERSION",
    "DecodeMemoryProfile",
    "CUDAMemorySample",
    "CuTileDispatchResult",
    "CuTileRuntimeInfo",
    "VLLMGenerateResult",
    "VLLMRuntimeInfo",
    "DeepSeekMemoryAdapter",
    "DecodeConfig",
    "DecodeBackendMatrix",
    "DeepDebugReport",
    "DebuggerV2Report",
    "DEBUGGER_V2_SCHEMA_VERSION",
    "ENV_CERTIFICATION_SCHEMA_VERSION",
    "EnvironmentCertificationReport",
    "FalconMemoryAdapter",
    "DecodeResult",
    "DriftReport",
    "FamilyAdapterCapabilities",
    "FusedDecodeBenchmark",
    "GemmaMemoryAdapter",
    "GPTNeoXMemoryAdapter",
    "GraphSpec",
    "HARDWARE_PROFILE_SCHEMA_VERSION",
    "HFCachePlan",
    "HF_CACHE_CERTIFICATION_SCHEMA_VERSION",
    "HF_CACHE_PROFILE_SCHEMA_VERSION",
    "HFCacheCertificationCase",
    "HFCacheCertificationReport",
    "HFCacheCertificationResult",
    "HFCachePolicy",
    "HFCachePolicyDecision",
    "HFGenerateRun",
    "HFGenerateIntegrationPlan",
    "HFMemoryFirstBenchmark",
    "HardwareProfile",
    "KernelCertificationCase",
    "KernelCertificationReport",
    "KernelCertificationResult",
    "KERNEL_PROMOTION_SCHEMA_VERSION",
    "KernelPolicy",
    "KernelPromotionDecision",
    "KernelPromotionRequirements",
    "LOCAL_MODEL_FIXTURE_SCHEMA_VERSION",
    "InferenceSuiteResult",
    "KVCacheConfig",
    "KVQualityResult",
    "KernelConfig",
    "LayerDriftRecord",
    "LogitQualityResult",
    "MemoryEstimate",
    "MemoryFirstGenerateResult",
    "MemoryFirstHFAdapter",
    "MemoryFirstHFConfig",
    "MemoryPlanner",
    "MemoryPolicy",
    "ModelArchitectureInfo",
    "ModelCertificationOutcome",
    "ModelCertificationTarget",
    "LocalModelFixture",
    "LocalModelHarnessReport",
    "LocalModelMatch",
    "OOMRunResult",
    "OOMStrategy",
    "OperationSpec",
    "OptimizationFinding",
    "OptimizationReport",
    "PackedQKVAttentionAdapter",
    "PagedKVCache",
    "PatchReport",
    "PhiMemoryAdapter",
    "QuantizedAttentionDispatch",
    "QuantizedCacheCertificationProfile",
    "DEFAULT_QUANTIZED_CACHE_PROFILES",
    "QuantizedStaticKVCache",
    "StaticKVCache",
    "TensorLifetime",
    "TensorSpec",
    "TokenQualityResult",
    "TorchTrace",
    "TransformerConfig",
    "ServingBenchmarkResult",
    "ServingRun",
    "analyze_qk_attention",
    "apply_rope",
    "attention_stats_to_text",
    "benchmark_callable",
    "benchmark_decode",
    "benchmark_forward",
    "benchmark_fused_decode_attention",
    "benchmark_decode_backend_matrix",
    "benchmark_inference_suite",
    "benchmark_memory_first_hf_generate",
    "benchmark_serving_paths",
    "benchmark_dashboard_from_files",
    "certify_hf_cache",
    "certify_hf_cache_model",
    "certify_environment",
    "certify_model_matrix",
    "certify_cutile_decode_attention",
    "certify_quantized_attention",
    "choose_memory_policy",
    "chunked_cross_entropy",
    "collect_attention_stats",
    "collect_run_metadata",
    "compare_benchmark_records",
    "compare_benchmarks",
    "compare_inference_suites",
    "compare_record_sets",
    "compare_layer_drift",
    "build_deep_debug_report",
    "debugger_v2_to_html",
    "compare_logits",
    "compare_report_to_html",
    "compare_token_sequences",
    "assert_no_regressions",
    "assert_hf_benchmark_passed",
    "assert_hf_cache_certified",
    "default_backend_registry",
    "default_generation_strategies",
    "default_kernel_certification_cases",
    "default_kernel_policy",
    "default_model_certification_targets",
    "DEFAULT_LOCAL_MODEL_FIXTURES",
    "dequantize_int8_per_token",
    "dequantize_uint8_per_token",
    "dtype_size_bytes",
    "estimate_transformer_memory",
    "evaluate_attention_kv_quality",
    "evaluate_int8_kv_quality",
    "evaluate_kv_quantization_quality",
    "format_bytes",
    "greedy_decode",
    "infer_findings",
    "inspect_model",
    "is_oom_error",
    "kernel",
    "linear_cross_entropy",
    "load_hf_model",
    "make_transformers_cache_from_model",
    "memory_first_generate",
    "optimize_hf_model",
    "optimize_llama_qwen_attention",
    "parse_bytes",
    "plan_hf_cache",
    "preset_config",
    "qkv_rope_attention",
    "qkv_rope_attention_cached",
    "quantize_int8_per_token",
    "quantize_uint8_per_token",
    "quantized_kv_attention",
    "production_hf_adapter_matrix",
    "promote_profile",
    "register_memory_adapter",
    "read_benchmark_csv",
    "read_benchmark_files",
    "read_benchmark_json",
    "record_from_benchmark",
    "records_from_suite",
    "resolve_kv_storage_dtype",
    "rms_norm",
    "rms_norm_manual_backward",
    "run_with_oom_fallback",
    "sample_next_token",
    "scan_local_model_fixtures",
    "scaled_dot_product_attention",
    "scoreboard_to_html",
    "select_quantized_cache_profile",
    "select_quantized_attention_backend",
    "swiglu",
    "trace_forward",
    "trace_interactive_to_html",
    "trace_timeline_to_html",
    "trace_to_html",
    "triton_apply_rope",
    "triton_available",
    "triton_dequantize_int8_per_token",
    "triton_dequantize_uint8_per_token",
    "triton_fused_int8_kv_attention",
    "triton_fused_uint8_kv_attention",
    "triton_fused_int8_paged_kv_attention",
    "triton_quantize_int8_per_token",
    "triton_quantize_uint8_per_token",
    "triton_rms_norm",
    "triton_swiglu_activation",
    "write_benchmark_csv",
    "write_benchmark_json",
    "write_quantized_cache_profiles",
    "assert_quality_regression",
    "compare_memory_profiles",
    "detect_hf_adapter_info",
    "detect_cutile_runtime",
    "detect_hardware_profile",
    "detect_vllm_runtime",
    "decide_kernel_promotion",
    "install_memory_first_generate",
    "adapter_satisfies_contract",
    "list_memory_adapters",
    "load_backend_entrypoints",
    "load_quantized_cache_profiles",
    "memory_first_generate_hf",
    "profile_decode_memory",
    "run_quality_regression",
    "run_vllm_generate",
    "select_kernel_policy",
    "select_hf_cache_policy",
    "supports_transformers_cache_api",
    "triton_fused_uint8_paged_kv_attention",
    "write_memory_profile_html",
    "hf_cache_certification_to_html",
    "write_deep_debug_html",
    "hf_adapter_matrix_to_text",
    "write_debugger_v2_html",
    "write_compare_html",
    "write_benchmark_dashboard_html",
    "write_interactive_html",
    "write_scoreboard_html",
    "write_timeline_html",
    "write_trace_html",
    "vllm_backend_plugin",
    "__version__",
]
